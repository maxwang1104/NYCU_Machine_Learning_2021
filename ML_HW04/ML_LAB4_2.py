...
@jit
def GetData():	
    images = np.fromfile('train-images.idx3-ubyte', dtype='ubyte')
    X = (images[4 * 4:].astype('float64').reshape(60000, -1) / 128).astype('int32')
    labels = np.fromfile('train-labels.idx1-ubyte',dtype='ubyte')
...
def Init():
    ...
    P = np.random.rand(10, 28 *28).astype(np.float64)
    P_prev = np.zeros((10, 28 *28), dtype=np.float64)
    return LAMBDA, P, P_prev

@jit
def E(X, LAMBDA, P):
    Z = np.zeros((60000, 10), dtype=np.float64)
...
def AssignLabel(X, labels, LAMBDA, P):
    counting = np.zeros((10, 10), dtype=np.int_)
    mapping = np.zeros((10), dtype=np.int_)
    for i in range(60000):
...
def PrintImagination(P, mapping, labeled):
    for i in range(10):
        ...
        for r in range(28):
            for c in range(28):
                char = '1' if P[class_idx, r * 28 + c] >= 0.5 else '0'
                print(char, end=' ')
            print()
        print()
...
def PrintResult(X, labels, LAMBDA, P, mapping, iteration):
    ...
    mapping_inverse = np.zeros((10), dtype=np.int_)
    for i in range(10):
...
if __name__ == '__main__':
    ...
    max_iteration = 200
    condition = 0
    mapping = np.array([i for i in range(10)], dtype=np.int_)
