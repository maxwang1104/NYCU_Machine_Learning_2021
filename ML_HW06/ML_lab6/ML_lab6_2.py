...
def draw(image, imagename, cluster, iteration, cut, FOLDER):
	...
	plt.suptitle(title)
	plt.tight_layout()
	plt.savefig(f'D:\Download_vivaldi\ML_HW06\ML_HW06\spectral-clustering/{FOLDER}/{imagename}_{CLUSTER_NUM}-cluster_{cut}_{iteration}.png')
...
def drawEigenSpace(eigen, cluster, cut):
	...
	plt.title(title)
	plt.tight_layout()
	plt.savefig(f'D:\Download_vivaldi\ML_HW06\ML_HW06\spectral-clustering/{imagename}_{CLUSTER_NUM}-cluster_{cut}_eigenspace.png')

def kMeans(image, imagename, eigen, cut, method, FOLDER):
	iteration = 0
	centroids, cluster = init(eigen, method)
	draw(image, imagename, cluster, iteration, cut, FOLDER)
	while 1:
		iteration += 1
		pre_cluster = cluster
...
if __name__ == "__main__":
	...
	if len(sys.argv) < 5:
		...
		if not os.path.exists(f'D:\Download_vivaldi\ML_HW06\ML_HW06\spectral-clustering\{FOLDER}'):
			os.mkdir(f'D:\Download_vivaldi\ML_HW06\ML_HW06\spectral-clustering\{FOLDER}')
		...
		if cut == 'normalized':
			...
			kMeans(image, imagename, T, cut, method, FOLDER)
		elif cut == 'ratio':
			L = D - kernel
			eigen_val, eigen_vec = np.linalg.eig(L)
			idx = np.argsort(eigen_val)
			eigen_vec = eigen_vec[:, idx]
			U = eigen_vec[:, 1:CLUSTER_NUM+1].real
			kMeans(image, imagename, U, cut, method, FOLDER)
		else:
			print('unknown cut')
